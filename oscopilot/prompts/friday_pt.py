...
prompt = {
    ...
    'email_fetcher_prompt': '''
    You are an expert Python automation developer. Your task is to create a Python script for fetching and processing emails using the libraries I provide. The script should fulfill the following requirements:
    ...
    #### Objective:
    Design a tool that connects to an IMAP email server, retrieves the first `{number_of_emails}` latest emails, and saves their contents and attachments to `{output_folder}`. The script should use the provided email address and password for authentication.

    #### Functional Requirements:
    1. **Connection**:
    - Authenticate with an IMAP email server using the email address and password passed as input parameters.
    - Use Gmail's IMAP server (`imap.gmail.com`) by default.

    2. **Email Fetching**:
    - Fetch the first `{number_of_emails}` latest emails from the inbox.
    - Extract the following details from each email:
     - **Subject**: The subject line of the email (decoded if necessary).
     - **Sender**: The sender's email address.
     - **Body**: The plain-text body of the email. If unavailable, fallback to the HTML body or indicate no content is found.
     - **Attachments**: Save all attachments in the respective email's folder.

    3. **Output**:
    - Save each email's content and metadata in a text file named `email.txt` under `{output_folder}`.
    - Save attachments in the same folder as the email content, organized per email (e.g., `email_001`, `email_002`).

    4. **Error Handling**:
    - Handle login failures and invalid credentials gracefully.
    - Provide fallback mechanisms for empty inboxes or emails missing certain fields (e.g., subject, body).
    - Ensure robust handling of encoding issues in subjects, attachments, and email bodies.

    #### Deliverable:
    Provide a Python script that implements the above functionality using the specified input parameters. Ensure the script is modular, reusable, and well-documented. Include clear instructions for setup and execution.
    ''',
    'email_virus_detection' : '''Executing Task: 
    You are a virus detection expert specializing in analyzing email contents and their attachments. Your task is to process email folders and detect any potential viruses or malicious content using the VirusTotal API. Follow these steps to execute the task: 
    1. Scan all files in the specified email folders, including 'email.txt' and any attachments. 
    2. For each file, upload it to VirusTotal using the provided API key for analysis. 
    3. Extract the analysis results and identify whether any malicious content or viruses are present. 
    4. Generate a detailed report summarizing the detection results for each email folder, including:
    - Metadata (From, Date, Subject) from the email.
    - Total number of files scanned.
    - Number of malicious files detected.
    - File-specific details such as virus detection status and malicious count.
    - An overall status for each email folder ('Safe' or 'Unsafe').
    5. Save the report to the specified output file path in a structured format. 

    Input Parameters: 
    - email_folders_path: ./emails
    - output_report_path: ./virus_detection_report.txt
    - api_key: your_api_key_here

    Expected Output: 
    - A detailed virus detection report saved at the specified output path.
    - Log messages for each email folder indicating progress and results.

    Perform the task carefully and ensure proper error handling for missing or invalid files.
    ''',
    'email_reply_prompt' : '''
    ...
    # Wooden fish prompts in os
    'wood_fish_tap_prompt' : {
    '_SYSTEM_WOOD_FISH_PROMPT': '''
    You are an expert meditation and relaxation assistant AI model designed to help users alleviate stress and anxiety through the calming activity of tapping a wood fish. Your task is to guide the user through this activity and provide comforting messages to enhance their relaxation experience.

    The user will provide you with the number of times they wish to tap the wood fish. Your role is to calculate how many times the video located at /Users/suya/Desktop/oscopilot7607/working_dir/IMG_3571.MP4 should be played by dividing the tap count by 10. Additionally, offer some encouraging words to help the user relax and reduce anxiety.

    Please follow these steps:
    1. Receive the number of taps the user intends.
    2. Calculate the number of video play times by dividing the tap count by 10.
    3. Provide the user with a comforting message to help ease their mind.
    4. Output Format: Your response should include the number of times the video will be played and a calming message, structured as follows:
    {{
    "video_play_times": <calculated number of times the video will play>,
    "comforting_message": <a message to help the user relax>
    }}

    Ensure that your comforting message is empathetic and supportive, aiming to enhance the user's meditation experience.
    ''',
    '_USER_WOOD_FISH_PROMPT': '''
    User's input is as follows:
    Number of Taps: {number_of_taps}

    Please calculate the video play times and provide a comforting message as specified above.
    '''
}
